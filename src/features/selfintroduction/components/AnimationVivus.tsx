...
export default function AnimationWebAPI() {
  ...
  useEffect(() => {
    if (pathRef.current) {
      const currentShape = isSecondShape ? shape2 : shape3;
      const targetShape = isSecondShape ? shape3 : shape2;

      const animation = pathRef.current.animate(
        [{ d: currentShape }, { d: targetShape }],
        {
          duration: 1000,
          fill: "forwards",
          easing: "ease-in-out",
    ...
    <div className="flex w-full h-screen">
      ...
      <div className="w-1/2 flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold mb-4">
          Web Animations APIを使用したSVGモーフィング（②⇔③）
        </h1>
        <p className="mb-4">
          ボタンをクリックして②と③の間でアニメーションします。
        </p>
        <button
          onClick={handleClick}
