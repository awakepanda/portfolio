...
export default function AnimationD3Interpolate() {
  ...
  useEffect(() => {
    if (pathRef.current) {
      const interpolator = interpolate(
        isSecondShape ? shape2 : shape3,
        isSecondShape ? shape3 : shape2,
      );
      const duration = 2000; // アニメーション時間は2秒のまま
      ...
      const animate = (currentTime: number) => {
        ...
        if (pathRef.current) {
          // easeCubicInOut関数を使用してアニメーションをスムーズに
          const easedProgress = easeCubicInOut(progress);
          pathRef.current.setAttribute("d", interpolator(easedProgress));
        }
    ...
    <div className="flex w-full h-screen">
      <div className="w-1/2 flex items-center justify-center">
        ...
        >
          ...
          <path ref={pathRef} d={shape2} fill="white" />
        </svg>
      </div>
